...
ava_1.default("path validations", (t) => {
    ...
    for (const validPath of validPaths) {
        t.truthy(configUtils.validateAndSanitizePath(validPath, propertyName, configFile, logging_1.getRunnerLogger(true)));
    }
    for (const invalidPath of invalidPaths) {
        t.throws(() => configUtils.validateAndSanitizePath(invalidPath, propertyName, configFile, logging_1.getRunnerLogger(true)));
    }
});
ava_1.default("path sanitization", (t) => {
    // Dummy values to pass to validateAndSanitizePath
    const propertyName = "paths";
    const configFile = "./.github/codeql/config.yml";
    // Valid paths are not modified
    t.deepEqual(configUtils.validateAndSanitizePath("foo/bar", propertyName, configFile, logging_1.getRunnerLogger(true)), "foo/bar");
    // Trailing stars are stripped
    t.deepEqual(configUtils.validateAndSanitizePath("foo/**", propertyName, configFile, logging_1.getRunnerLogger(true)), "foo/");
});
